...
- **联系人权限调整**：owner可点击联系人角色标签切换editor/viewer权限，新增updateRole API
- **宝宝逻辑删除**：宝宝表新增"状态"字段（正常/删除），删除宝宝改为设置状态为"删除"而非物理删除，同时清理关联记录
- **审批未通过状态**：账号状态枚举新增"审批未通过"，区别于"冻结"
- **年龄选择器优化**：右侧月份独立滚动不受左侧限制，移除"筛选整个X岁"按钮，"出生"改为"0岁"，左侧双击直接筛选整岁，整岁生日加🎂图标，修复非当前岁显示12个月问题
