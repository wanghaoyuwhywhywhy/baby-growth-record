...
| 权限 | 单选 | view / edit / admin / superadmin |
| 状态 | 单选 | 正常 / 冻结 / 删除 / 待审批 |
| 最后修改时间 | 日期 | 最后修改时间戳 |
...
- **邀请码**：宝宝创建者可生成邀请码（INV-XXXXXX格式）邀请家属
  ...
  - 未注册用户先注册再输入邀请码
- **四级权限**：view/edit/admin/superadmin
  - 仅superadmin可审批账号、管理所有账号
  - 默认admin账号升级为superadmin
- **关联表增强**：新增关系字段（11个选项）和邀请码字段
- **数据隔离完善**：所有数据API按账号-宝宝关联过滤
- **宝宝删除权限**：仅宝宝的owner（创建者）可删除宝宝
- **前端auth**：新增getAuthBabyRelations/getAuthBabyLinkRoles/isSuperAdmin
